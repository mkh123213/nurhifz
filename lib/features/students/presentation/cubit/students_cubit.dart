...
class StudentsCubit extends Cubit<StudentsState> {
  ...
  Future<void> updateStudentLevel(String studentId, String level) async {
    try {
      await _studentsRepo.update(studentId, {'level': level});
      final updated = state.students
          .map((s) => s.id == studentId ? s.copyWith(level: level) : s)
          .toList();
      emit(state.copyWith(students: updated));
    } catch (_) {
      emit(state.copyWith(errorKey: 'error_unknown'));
    }
  }
}
