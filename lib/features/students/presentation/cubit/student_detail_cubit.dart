...
class StudentDetailCubit extends Cubit<StudentDetailState> {
  ...
  Future<void> load(String studentId) async {
    ...
    try {
      ...
      emit(state.copyWith(
          status: DetailStatus.error, errorKey: 'error_unknown'));
    }
  }
  ...
  Future<void> updateSession(String sessionId, String studentId, Map<String, dynamic> data) async {
    emit(state.copyWith(isAddingSession: true));
    try {
      await _sessionsRepo.update(sessionId, data);
      final sessions = await _sessionsRepo.getByStudent(studentId);
      emit(state.copyWith(isAddingSession: false, sessions: sessions));
    } catch (_) {
      emit(state.copyWith(isAddingSession: false, errorKey: 'error_unknown'));
    }
  }
